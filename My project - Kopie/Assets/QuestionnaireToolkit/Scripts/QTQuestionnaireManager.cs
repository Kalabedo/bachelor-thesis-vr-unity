...
namespace QuestionnaireToolkit.Scripts
{
    ...
    public class QTQuestionnaireManager : MonoBehaviour
    {
        ...
        private int _oldDisplayMode = 0;
        private int _oldOrientation = 0;
        private int _oldColorScheme = 0;
        private int _oldDeviceType = 0;
        private bool _oldDynamicHeight = false;
        ...
        private GameObject _visiblePage = null;
        ...
        public float averageResultWReality = 0;
        public float averageResultWOReality = 0;
...
#pragma warning restore 618
        ...
        private void ReadJson(string jsonPath)
        {
            ...
            while (true)
            {
                ...
                if (pId != "")
                {
                    ...
                    var fullscreenText = N["qPages"][i]["fullscreen_text"].Value;
                    var showInstructionPanel = N["qPages"][i]["show_instruction_panel"].AsBool;
                    var instructionText = N["qPages"][i]["instruction_text"].Value;
                    ...
                    for (var j = 0; j < qItems.Count; j++)
                    {
                        ...
                        {
                            Debug.LogError("'_' is not supported as header name!");
                            break;
                        }
                        ...
                        switch (qType)
                        {
                            case "slider":
                                ...
                                currentImportedItem.GetComponent<QTSlider>().ImportSliderValues(mandatory, minValue, maxValue, wholeNumbers, showPanels, showIntermediatePanels, labelZero, labelQuarter, labelHalf, labelThreeQuarters, labelFull);
                                break;
                            case "dropdown":
                            ...
                            case "multiple_choice":
                                ...
                                for (var op = 0; op < options.Count; op++)
                                {
                                    currentImportedItem.GetComponent<QTMultipleChoice>().AddOption(true, this, mandatory, options[op]["answer_value"].Value, options[op]["answer_option"].Value);
                                }
                                break;
        ...
        private void WriteJson(string savePath)
        {
            ...
            foreach (var page in questionPages) 
            {
                ...
                jsonString += "{\"pId\":\"page" + pageCount + "\"," 
                              ...
                              + "\"fullscreen_text\":\"" + pageScript.fullscreenText.Replace("\n", "\\n") + "\","
                              + "\"show_instruction_panel\":\"" + pageScript.showTopPanel + "\","
                              + "\"instruction_text\":\"" + pageScript.instructionText.Replace("\n", "\\n") + "\","
                              + "\"qItems\":[";
                pageCount++;
        ...
        public void NextPage()
        {
            ...
            else
            {
                ...
                if (_currentPage < questionPages.Count-1) // show only next page
                {
                    ...
                    if (_currentPage == 2)
                    {
                        //TODO wenn auf seite 3
                        GetAverageResult();
                        GetAverageWORealityResult();
                    }
                    ShowPage(_currentPage);
        ...
        public void BuildHeaderItems()
        {
            ...
            foreach (var page in questionPages)
            {
                try
                {
                    foreach (var question in page.GetComponent<QTQuestionPageManager>().questionItems)
                    {
                        if (question.CompareTag("QTText") || question.CompareTag("QTImage") || question.CompareTag("QTButton") || question.CompareTag("QTVideo")) break;
        ...
        {
            StreamWriter writer = new StreamWriter(userPath, true);
            List<int> linearScaleResults = new List<int>(); // own Code
                ...
                foreach (var question in page.GetComponent<QTQuestionPageManager>().questionItems)
                {
                    switch (question.tag)
                    {
                        case "QTLinearScale":
                            ...
                            int linearScaleResult = int.Parse(toggleGroup.ActiveToggles().FirstOrDefault().gameObject.name.Split('_')[0]);
                            linearScaleResults.Add(linearScaleResult);
                            line += linearScaleResult + ",";
                            break;
                        case "QTCheckboxes":
                        ...
                        case "QTCheckboxesGrid":
                            ...
                            for (var i = 0; i < checkboxGrid.childCount; i++)
                            {
                                ...
                                if (currChild.CompareTag("QTGridOption") && currChild.GetComponent<Toggle>().isOn)
                                {
        ...
        public void GetAverageResult()
        {
            List<int> linearScaleResults = new List<int>(); // own Code
                ...
                questionPages[1].SetActive(true); // re-enable all pages otherwise the items cannot be found
                foreach (var question in questionPages[1].GetComponent<QTQuestionPageManager>().questionItems)
                    {
                        switch (question.tag)
                        {
                            case "QTLinearScale":
                ...
                questionPages[1].SetActive(false);
            // questionPages[2].SetActive(true);
            // graphManager.setSizeOfBar();
            float averageResult = 0;
            int[] linearScaleResultsArray = linearScaleResults.ToArray();
                for (int i = 0; i < linearScaleResultsArray.Length; i++)
                {
                    averageResult += linearScaleResultsArray[i];
                }
                averageResultWReality = averageResult / linearScaleResultsArray.Length;

            // graphManager.setSizeOfBar1(averageResult);

        }

        public void GetAverageWORealityResult()
        {
            List<int> linearScaleResults = new List<int>(); // own Code
            ...
            float averageResult = 0;
            int[] linearScaleResultsArray = linearScaleResults.ToArray();
            linearScaleResultsArray[2] = 0;
            linearScaleResultsArray[3] = 0;
                for (int i = 0; i < linearScaleResultsArray.Length; i++)
                {
                    averageResult += linearScaleResultsArray[i];
                }
                averageResultWOReality = averageResult / (linearScaleResultsArray.Length - 2);

            // graphManager.setSizeOfBar2(averageResult);
